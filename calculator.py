...
def geocode_single(address: str, cache: dict) -> list | None:
    ...
    if result:
        save_to_geocache(address, result[0], result[1])
    else:
        save_to_geocache(address, "", "")  # record failure so it is not retried
    load_geocache.clear()
    return result
...
def orlen_on_date(target_date: date, lookup: dict[str, float], _sorted_keys=None) -> float | None:
...
# Exclude planned / future deliveries — applied outside the cache boundary so
# "today" is evaluated each run, not frozen at first-call date.
df = df[df["Data dostawy"] < pd.Timestamp(_today)]
...
df["dist_km"] = haversine_km_vec(
    dest_lat, dest_lon, df["lat"].to_numpy(), df["lon"].to_numpy()
)
...
_orlen_keys = sorted(orlen_lookup.keys())
nearby["diesel_pln_m3"] = nearby["Data dostawy"].apply(
    lambda d: orlen_on_date(d.date(), orlen_lookup, _orlen_keys) if pd.notna(d) else None
)
