...
def orlen_on_date(target_date, lookup: dict, _sorted_keys=None):
    if not lookup:
        return None
    target_str = target_date.isoformat()
    hit = lookup.get(target_str)
    if hit is not None:
        return hit
    keys = _sorted_keys if _sorted_keys is not None else sorted(lookup.keys())
    idx = bisect.bisect_right(keys, target_str) - 1
    if idx >= 0:
...
def load_geocache() -> dict:
    ...
    for r in records:
        if r["lat"] != "" and r["lon"] != "":
            cache[r["address"]] = [float(r["lat"]), float(r["lon"])]
        else:
            cache[r["address"]] = None  # failed attempt — skip on future runs
    return cache
...
def geocode_addresses(addresses, cache):
    ...
    for i, addr in enumerate(new_addresses):
        ...
        if result:
            ...
            save_to_geocache(addr, "", "")  # record failure so it is not retried
        bar.progress((i + 1) / len(new_addresses), text=f"Geocoding {i+1}/{len(new_addresses)}: {addr[:60]}…")
...
_lat_map = {a: (c[0] if c else None) for a, c in cache.items()}
_lon_map = {a: (c[1] if c else None) for a, c in cache.items()}
df["lat"] = df["Miejscowosc"].map(_lat_map)
df["lon"] = df["Miejscowosc"].map(_lon_map)
...
_min_ts = df["Data dostawy"].min()
date_min = _min_ts.date() if pd.notna(_min_ts) else _date_today
...
for _, row in addr_groups.iterrows():
    ...
    for _, dr in addr_rows.iterrows():
        ...
        <tr style="border-bottom:1px solid #f0f0f0">
            ...
            <td style="padding:3px 6px;text-align:right">{_fmt(dr['PLN / t'], '.2f')}</td>
            <td style="padding:3px 6px;text-align:right">{_fmt(dr['PLN / km'], '.2f')}</td>
            <td style="padding:3px 6px">{dr['Features.TransportKontrahent:Kod'] if pd.notna(dr['Features.TransportKontrahent:Kod']) else '—'}</td>
            <td style="padding:3px 6px;text-align:right">{_fmt(dr['Tony'], '.2f')}</td>
        </tr>"""
    ...
    <div style="font-family:Arial,sans-serif;font-size:12px;width:580px;line-height:1.5">
        ...
        <div style="color:#555;margin-bottom:6px"><b>Deliveries:</b> {row['delivery_count']} &nbsp;|&nbsp;
            <b>Dates:</b> {date_str} &nbsp;|&nbsp; <b>Distance:</b> {(format(row['km'], '.0f') + ' km') if pd.notna(row['km']) else '—'}</div>
...
_METRIC_OPTIONS = {
    "PLN / t":    "Average price per tonne (PLN/t)",
    "PLN / km":   "Average price per km (PLN/km)",
    "Δ PLN / t":  "Price premium per tonne — avg excl. min − min (PLN/t)",
    "Δ PLN / km": "Price premium per km — avg excl. min − min (PLN/km)",
}

def _weighted_premium(group_df, price_col):
    """
    For each destination in group_df, compute premium = avg_excl_min − min,
    then return the weighted average of those premiums weighted by total tonnes
    per destination.
    """
    results = []
    for _dest, dest_df in group_df.groupby("Miejscowosc"):
        s = dest_df[price_col].dropna()
        if len(s) < 2:
            continue
        dest_min  = s.min()
        above_min = s[s > dest_min]
        if above_min.empty:
            continue
        avg_excl  = above_min.mean()
        premium   = avg_excl - dest_min
        weight    = dest_df["Tony"].sum()
        results.append((premium, weight))
    if not results:
        return 0.0
    total_weight = sum(w for _, w in results)
    if total_weight == 0:
        return 0.0
    return sum(p * w for p, w in results) / total_weight
...
with pc1:
    metric_toggle = st.radio(
        ...
        format_func=lambda x: _METRIC_OPTIONS[x],
        horizontal=True,
        label_visibility="collapsed",
    )
with pc2:
...
if len(filtered) == 0:
    ...
    _is_premium = metric_toggle.startswith("Δ")
    _base_col   = "PLN / t" if metric_toggle in ("PLN / t", "Δ PLN / t") else "PLN / km"

    if _is_premium:
        metric_label = f"Weighted premium over min (PLN/{'t' if _base_col == 'PLN / t' else 'km'})"
        _rows = []
        for _grp_val, _grp_df in filtered.groupby(price_group):
            _rows.append({
                price_group_label: _grp_val,
                metric_label: _weighted_premium(_grp_df, _base_col),
            })
        price_chart = (
            pd.DataFrame(_rows)
            .sort_values(metric_label, ascending=False)
        )
        _bar_color = px.colors.qualitative.Vivid
        _caption = (
            "Per destination: premium = avg(excluding cheapest delivery) − cheapest delivery. "
            "Bars show the weighted average across all destinations, weighted by tonnes delivered."
        )
    else:
        metric_label = "Avg PLN / tonne" if _base_col == "PLN / t" else "Avg PLN / km"
        price_chart = (
            filtered.groupby(price_group)[_base_col]
            .mean()
            .reset_index()
            .rename(columns={price_group: price_group_label, _base_col: metric_label})
            .sort_values(metric_label, ascending=False)
        )
        _bar_color = px.colors.qualitative.Set2
        _caption = None
    ...
    with st.expander("🔢 Price Analysis — working out"):
        if _is_premium:
            _unit = "t" if _base_col == "PLN / t" else "km"
            for _grp_val, _grp_df in filtered.groupby(price_group):
                st.markdown(f"**{price_group_label}: {_grp_val}**")
                _dest_rows = []
                _total_w = 0.0
                _weighted_sum = 0.0
                for _dest, _dest_df in _grp_df.groupby("Miejscowosc"):
                    s = _dest_df[_base_col].dropna()
                    _dest_min = s.min() if not s.empty else None
                    _above = s[s > _dest_min] if _dest_min is not None else pd.Series(dtype=float)
                    _avg_excl = _above.mean() if not _above.empty else None
                    _prem = (_avg_excl - _dest_min) if _avg_excl is not None else None
                    _w = _dest_df["Tony"].sum()
                    _dest_rows.append({
                        "Destination": _dest,
                        f"Min PLN/{_unit}": round(_dest_min, 2) if _dest_min is not None else "—",
                        f"Avg excl. min PLN/{_unit}": round(_avg_excl, 2) if _avg_excl is not None else "—",
                        f"Premium PLN/{_unit}": round(_prem, 2) if _prem is not None else "—",
                        "Tonnes (weight)": round(_w, 2),
                        "Contribution": f"{round(_prem * _w, 2)} / {round(_w, 2)}" if _prem is not None else "—",
                    })
                    if _prem is not None:
                        _weighted_sum += _prem * _w
                        _total_w += _w
                _wa = _weighted_sum / _total_w if _total_w > 0 else 0.0
                st.dataframe(pd.DataFrame(_dest_rows), use_container_width=True, hide_index=True)
                st.markdown(
                    f"→ Weighted premium = {round(_weighted_sum, 2)} ÷ {round(_total_w, 2)} = **{_wa:.2f} PLN/{_unit}**"
                )
                st.divider()
        else:
            _unit = "tonne" if _base_col == "PLN / t" else "km"
            for _grp_val, _grp_df in filtered.groupby(price_group):
                st.markdown(f"**{price_group_label}: {_grp_val}**")
                _dest_rows = []
                for _dest, _dest_df in _grp_df.groupby("Miejscowosc"):
                    ...
                    _dest_rows.append({
                        "Destination": _dest,
                        "Deliveries": len(s),
                        f"PLN/{_unit} values": ", ".join(f"{v:.2f}" for v in s.sort_values()),
                        f"Avg PLN/{_unit}": round(s.mean(), 2) if not s.empty else "—",
                    })
                _grp_avg = _grp_df[_base_col].dropna().mean()
                st.dataframe(pd.DataFrame(_dest_rows), use_container_width=True, hide_index=True)
                st.markdown(f"→ Group average = **{_grp_avg:.2f} PLN/{_unit}**")
                st.divider()
...
with _tg_c2:
    excl_min_t  = st.toggle("Exclude tonnes at min PLN/t",  key="excl_min_t",
                             help="Subtracts tonnes delivered at the minimum PLN/t price per destination from the total.")
    excl_min_km = st.toggle("Exclude tonnes at min PLN/km", key="excl_min_km",
                             help="Subtracts tonnes delivered at the minimum PLN/km price per destination from the total.")
...
if len(filtered) == 0:
    ...
    def _tonnes_excl_min(group_df, price_col):
        """Total tonnes minus tonnes delivered at the minimum price per destination."""
        total = group_df["Tony"].sum()
        min_tonnes = 0.0
        for _dest, dest_df in group_df.groupby("Miejscowosc"):
            s = dest_df[price_col].dropna()
            if s.empty:
                continue
            dest_min = s.min()
            min_tonnes += dest_df.loc[dest_df[price_col] == dest_min, "Tony"].sum()
        return max(total - min_tonnes, 0.0)

    if excl_min_t or excl_min_km:
        _price_col  = "PLN / t" if excl_min_t else "PLN / km"
        _y_label    = (
            "Tonnes excl. min PLN/t deliveries" if excl_min_t
            else "Tonnes excl. min PLN/km deliveries"
        )
        _rows = []
        for _grp_val, _grp_df in filtered.groupby(tonnes_group):
            _rows.append({group_label: _grp_val, _y_label: _tonnes_excl_min(_grp_df, _price_col)})
        tonnes_chart = pd.DataFrame(_rows).sort_values(_y_label, ascending=False)
        _t_caption = (
            f"Tonnes delivered at the minimum PLN/{'t' if excl_min_t else 'km'} price "
            "per destination have been removed from each bar."
        )
    else:
        _y_label = "Total Tonnes"
        tonnes_chart = (
            filtered.groupby(tonnes_group)["Tony"]
            .sum()
            .reset_index()
            .rename(columns={tonnes_group: group_label, "Tony": _y_label})
            .sort_values(_y_label, ascending=False)
        )
        _t_caption = None
    ...
    if _t_caption:
        st.caption(_t_caption)

    with st.expander("🔢 Total Tonnes — working out"):
        if excl_min_t or excl_min_km:
            _pc = "PLN / t" if excl_min_t else "PLN / km"
            _pc_label = "PLN/t" if excl_min_t else "PLN/km"
            for _grp_val, _grp_df in filtered.groupby(tonnes_group):
                st.markdown(f"**{group_label}: {_grp_val}**")
                _dest_rows = []
                _total_all = _grp_df["Tony"].sum()
                _total_min_t = 0.0
                for _dest, _dest_df in _grp_df.groupby("Miejscowosc"):
                    s = _dest_df[_pc].dropna()
                    _dest_total = _dest_df["Tony"].sum()
                    if s.empty:
                        _dest_rows.append({
                            "Destination": _dest,
                            f"Min {_pc_label}": "—",
                            "Total Tonnes": round(_dest_total, 2),
                            "Tonnes at min price": "—",
                            "Remaining Tonnes": round(_dest_total, 2),
                        })
                        continue
                    _dmin = s.min()
                    _min_t = _dest_df.loc[_dest_df[_pc] == _dmin, "Tony"].sum()
                    _remaining = max(_dest_total - _min_t, 0.0)
                    _total_min_t += _min_t
                    _dest_rows.append({
                        "Destination": _dest,
                        f"Min {_pc_label}": round(_dmin, 2),
                        "Total Tonnes": round(_dest_total, 2),
                        "Tonnes at min price": round(_min_t, 2),
                        "Remaining Tonnes": round(_remaining, 2),
                    })
                st.dataframe(pd.DataFrame(_dest_rows), use_container_width=True, hide_index=True)
                st.markdown(
                    f"→ Total: {round(_total_all, 2)} − min-price tonnes: {round(_total_min_t, 2)}"
                    f" = **{round(max(_total_all - _total_min_t, 0.0), 2)} t**"
                )
                st.divider()
        else:
            for _grp_val, _grp_df in filtered.groupby(tonnes_group):
                ...
                for _dest, _dest_df in _grp_df.groupby("Miejscowosc"):
                    _dest_rows.append({
                        "Destination": _dest,
                        "Deliveries": len(_dest_df),
                        "Tonnes per delivery": ", ".join(f"{v:.2f}" for v in _dest_df["Tony"].dropna()),
                        "Total Tonnes": round(_dest_df["Tony"].sum(), 2),
                    })
                st.dataframe(pd.DataFrame(_dest_rows), use_container_width=True, hide_index=True)
                st.markdown(f"→ Group total = **{round(_grp_df['Tony'].sum(), 2)} t**")
                st.divider()
...
if len(filtered) == 0:
    ...
    _orlen_keys = sorted(_orlen.keys())
    _tbl["ORLEN Diesel (PLN/L)"] = _tbl["Data dostawy"].apply(
        lambda d: orlen_on_date(d.date(), _orlen, _orlen_keys) if pd.notna(d) else None
    )

    # ── Overpaid calculation per destination ─────────────────────────────────
    # For each destination:
    #   premium      = avg(PLN/t excl. all minimum-priced deliveries) − min
    #   tonnes_excl  = total tonnes of deliveries above the minimum price
    #   overpaid     = premium × tonnes_excl
    # Every row belonging to that destination shows the same destination total.
    def _dest_overpaid(grp, col):
        s = grp[col].dropna()
        if s.empty:
            return 0.0
        mn = s.min()
        above_mask = grp[col] > mn
        above_prices = grp.loc[above_mask, col].dropna()
        if above_prices.empty:
            return 0.0
        premium = above_prices.mean() - mn
        tonnes_excl = grp.loc[above_mask, "Tony"].sum()
        return premium * tonnes_excl

    _op_t  = filtered.groupby("Miejscowosc").apply(_dest_overpaid, "PLN / t").rename("Overpaid PLN/t (PLN)")
    _op_km = filtered.groupby("Miejscowosc").apply(_dest_overpaid, "PLN / km").rename("Overpaid PLN/km (PLN)")
    _tbl = _tbl.join(_op_t, on="Miejscowosc").join(_op_km, on="Miejscowosc")
    ...
    _tbl_display = _tbl.rename(columns={
        ...
        "Overpaid PLN/t (PLN)", "Overpaid PLN/km (PLN)",
    ]].copy()
    ...
    _total_op_t  = _tbl.groupby("Miejscowosc")["Overpaid PLN/t (PLN)"].first().sum()
    _total_op_km = _tbl.groupby("Miejscowosc")["Overpaid PLN/km (PLN)"].first().sum()
    ...
    _sc6.markdown("**Overpaid vs min PLN/t**")
    _sc6.markdown(f"💸 Total: **{_total_op_t:,.2f} PLN**")
    _sc6.caption("Sum of (price − dest min) × tonnes for above-minimum deliveries")

    _sc7.markdown("**Overpaid vs min PLN/km**")
    _sc7.markdown(f"💸 Total: **{_total_op_km:,.2f} PLN**")
    _sc7.caption("Sum of (price − dest min) × tonnes for above-minimum deliveries")
    ...
    st.dataframe(
        _tbl_display.style.format({
            ...
            "Overpaid PLN/t (PLN)":  "{:.2f}",
            "Overpaid PLN/km (PLN)": "{:.2f}",
        }, na_rep="—"),
